...
function be_ajax_load_more() {
	...
	if( $loop->have_posts() ): 
            while( $loop->have_posts() ): 
                $loop->the_post();
        displayPost($loop->post);
		//echo "<h1>".var_dump($loop)."</h1>";
            endwhile; 
        endif; 
        wp_reset_postdata();
	$data = ob_get_clean();
...
/*
 * 
 * Wol Akec - Our function for displaying the post
 * 
 */
...
function displayPost($post){
    ...
    $page = <<<EOT
		...
		<h1 class="entry-title">$title</h1>
            ...
            <div class="postName" style="display:none">$name</div>
...
function updateViews(){
    ...
    if($user->ID != 0){
        $id = $user->ID;
        $count = get_user_meta( $id, "viewCount", true ); 
        //If view count does not exist in metadata, create it, if it does, incrememnt and update the count
        if($count == ""){
            $count = 1;
            add_user_meta($id, "viewCount", $count);
        }else{
            $count++;
            update_user_meta($id, "viewCount",$count);
        }
    }else{
        return false;
    }
...
function updateUserData(){
    ...
    if($user->ID != 0){
        ...
        update_user_meta($id, "ipAddress",$ip);
        
        $browser = $_SERVER['HTTP_USER_AGENT'];
        update_user_meta($id, "browser",$browser);
